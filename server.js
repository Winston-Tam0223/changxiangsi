...
app.get('/api/tasks', authMiddleware, async (req, res) => {
  ...
  let rows = data || [];
  // 普通用户按"可见任务"过滤；管理员与未设置的用户看到全部
  if (!isAdmin(req.user)) {
    const meta = req.user.user_metadata || {};
    const vt = meta.visible_tasks;
    if (Array.isArray(vt) && vt.length) {
      const set = new Set(vt.map(String));
...
app.post('/api/set-user-tasks', authMiddleware, async (req, res) => {
  if (!isAdmin(req.user)) return res.status(403).json({ error: '无权限' });
  const { userId, taskIds } = req.body || {};
  if (!userId || !Array.isArray(taskIds)) return res.status(400).json({ error: '参数不完整' });
  // 读取现有 user_metadata 再合并，避免覆盖 isAdmin / status
  const { data: cur, error: gErr } = await supabaseAdmin.auth.admin.getUserById(userId);
  if (gErr || !cur) return res.status(500).json({ error: '未找到该用户' });
  const meta = cur.user_metadata || {};
  const { error } = await supabaseAdmin.auth.admin.updateUserById(userId, {
    user_metadata: { ...meta, visible_tasks: taskIds.map(String) }
  });
  if (error) return res.status(500).json({ error: error.message });
  res.json({ ok: true });
});
